...
import { faBolt, faFireExtinguisher, faQuestionCircle, faSyncAlt } from "@fortawesome/free-solid-svg-icons";
...
const getWildcardVisuals = (name: string): { icon: IconDefinition; color: string } => {
  const normalized = name.toLowerCase();
  const match = Object.entries(wildcardIconMap).find(([key]) => normalized.includes(key));
  if (match) return match[1];
  return { icon: faQuestionCircle, color: "#a855f7" };
};

export const WildcardProductCard = ({ item, quantity, onQuantityChange, onBuy, maxQuantity }: WildcardProductCardProps) => {
  const { icon, color } = getWildcardVisuals(item.name);

  return (
    <div className="bg-gray-900 rounded-lg p-4 flex-shrink-0 w-[260px] border-2 border-purple-500 flex flex-col gap-3">
      <span className="text-xs uppercase tracking-widest text-purple-200">Wildcard</span>
      <div className="flex flex-col items-center text-center gap-2">
        ...
        <div>
          <h3 className="text-white tracking-wider text-lg font-semibold">{item.name}</h3>
      ...
      <div className="flex items-center justify-between text-sm text-gray-300">
        <p>
          Tenés: <span className="text-white font-semibold">{item.currentQuantity}</span>
        </p>
