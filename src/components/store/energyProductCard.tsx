...
export const EnergyProductCard = ({
  ...
  return (
    <div
      className={`bg-gray-900 rounded-lg p-4 flex-shrink-0 w-[260px] border-2 ${canBuy ? "border-yellow-500" : "border-gray-600 opacity-70"} flex flex-col gap-3`}
    >
      <span className="text-xs uppercase tracking-widest text-yellow-200">Energía</span>
      <div className="flex flex-col items-center text-center gap-2">
        <div className="w-20 h-20 rounded-lg border-2 border-yellow-500 flex items-center justify-center ">
          <img
            src={pilaBoltImg}
            alt="batería"
        ...
        <div>
          <h3 className="text-white tracking-wider text-lg font-semibold">Recarga inmediata</h3>
          {canBuy ? (
            <p className="text-gray-400 text-xs">Seguí jugando sin esperar la recarga automática.</p>
          ) : (
      ...
      {energyInfo ? (
        <>
          <div className="flex items-center justify-between text-sm text-gray-300">
            <p>
              Tenés: <span className="text-white font-semibold">{energyInfo.currentAmount}/{energyInfo.maxAmount}</span>
            </p>
        ...
        <p className="text-gray-400 text-sm">No se pudo cargar tu energía actual.</p>
      )}
